...
- `PATCH /api/todos/:id`：`{ title?, completed?, categoryId?, progress?, reminderEnabled?, reminderTime?, creatorEmail? }`
- `DELETE /api/todos/:id`
...
输入约束包括：标题 1–200 字符、分类名 1–80 字符、进度为 0–100 整数、时间为 `HH:MM`、提醒邮箱最多 320 字符。不存在的待办或分类返回 HTTP 404。
